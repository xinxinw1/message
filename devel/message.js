...
function runOnload(){
  if (isEmpty(name) || isEmpty(doc)){
    ...
    if (str.indexOf("latest") == -1 && str.indexOf("devel") == -1){
      ...
      if (str.indexOf("test") != -1)msg += "http://musiclifephilosophy.com/test/apps/message/latest/message.php";
      else msg += "http://musiclifephilosophy.com/codes/apps/message/latest/message.php";
      receiveData("error|" + encodeURIComponent(time) + "|" + encodeURIComponent(msg) + "\n");
    }
    checkConnection();
    ...
    $("source").onclick = function (){
      window.open('/codes/apps/message/docs/' + encodeURIComponent(doc), '_blank', 'height=550, width=900, top=150, left=200, menubar=no, resizable=yes, scrollbars=yes, status=no, toolbar=no');
    }
  }
}
...
function checkNewOnce(){
  ...
  Ajax.sendRequest(file, params, func, type);
}
